...
def run(input_dir, output_dir1, output_dir2, loc_grid, loc_cloud, region, resolution, ncritical, count_start, count_end):
    ...
    for n in range(len(path_list)):
        for m in range(len(path_list[n])):
            ...
            cls5 = TOA_REFLECTANCE(5, region, path, loc_cloud, count_start, count_end)
            cls6 = TOA_REFLECTANCE(6, region, path, loc_cloud, count_start, count_end)
...
# =============================================================================
                ...
                print('[{} / {}] REMOVED due to Cloud Contamination {}'.format(count, total_size, filename))
            count += 1      
...
def parse_opt():
    ...
    parser.add_argument('--resolution', type = float, default = 6.25, help = 'Grid Resolution: 6.25, 12.5, 25 km')
    parser.add_argument('--ncritical', type = int, default = 15000, help = 'Critical Sample Size') # 15000 for 6.25 km resolution
    parser.add_argument('--count_start', type = int, default = 1, help = 'Start Count')
    parser.add_argument('--count_end', type = int, default = 4000, help = 'End Count')
